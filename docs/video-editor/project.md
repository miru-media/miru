...
const { sync, webrtc, error } = useVideoEditorStore(id, console.error)
...
<ClientOnly>
  ...
  <VideoEditorDoc v-else-if="sync" :sync :onCloseProject="() => router.go('/video-editor')" v-model:name="name"/>
  <button
