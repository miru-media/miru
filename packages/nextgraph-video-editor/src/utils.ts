...
type WithGraphId<T extends Record<string, unknown>> = {
  '@graph': string
  '@id': string
} & {
  [P in keyof T]: T[P] extends Record<string, any> ? WithGraphId<T[P]> : T[P]
}

export const withEmptyGraphIds = <T extends Record<string, any>>(obj: T): WithGraphId<T> => {
  const newObj = { '@graph': '', '@id': '' } as unknown as WithGraphId<T>

  for (const key in obj)
    if (Object.hasOwn(obj, key)) {
      const value = obj[key]

      ;(newObj as any)[key] = typeof value === 'object' && value != null ? withEmptyGraphIds(value) : value
    }
