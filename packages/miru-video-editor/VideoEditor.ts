...
import type * as pub from './typings.d'
...
export class VideoEditor implements pub.VideoEditor {
  ...
  async createMediaAsset(source: string | Blob): Promise<MediaAsset> {
    return MediaAsset.fromSource(uid(), source)
  }
  ...
  selectPrevClip(seek = true) {
    const prev = this.#selection.value?.prev
    if (prev) this.#selectClip(prev, seek)
  }
  selectNextClip(seek = true) {
    const next = this.#selection.value?.next
    if (next) this.#selectClip(next, seek)
  }
  ...
  setClipFilter(clip: Clip, filter: VideoEffectAsset | undefined, intensity: number) {
    const from = clip.getSnapshot()
