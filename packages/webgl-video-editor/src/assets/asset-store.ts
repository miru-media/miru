...
import { useChangeCounter } from 'shared/utils/composables.ts'
...
export class FileSystemAssetStore extends EventTarget implements pub.VideoEditorAssetStore {
  ...
  readonly #changeCounter = useChangeCounter()
  ...
  constructor(generateId: () => string = uid) {
    ...
    this.on('asset:create', this.#changeCounter.inc)
    this.on('asset:delete', this.#changeCounter.inc)
  }

  values(): MapIterator<pub.AnyAsset> {
    this.#changeCounter.track()
